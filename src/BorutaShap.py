...
class BorutaShap:
    ...
    def Check_if_chose_train_or_test_and_train_model(self):
        ...
        if self.stratify is not None and not self.classification:
            raise ValueError(
                'Cannot take a strtified sample from continuos variable please bucket the variable and try again !')
    ...
    def fit(self, X, y, n_trials=20, random_state=0, sample=False,
            train_or_test='test', normalize=True, verbose=True, stratify=None, groups=None):
        ...
        groups: array
            array of Int, on for each row of X, indicating which group the (x, y) row belongs to. The group ids must be
            in increasing order to be compatible with xgboost. Only used in case of pairwise ranking.
